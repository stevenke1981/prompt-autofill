...
test.describe('Prompt Autofill smoke E2E', () => {
  test('discovery → editor → copy prompt', async ({ page, context }) => {
    await context.grantPermissions(['clipboard-read', 'clipboard-write']);
    await page.goto('/');
    await expect(page).toHaveTitle(/Prompt Autofill/i);
    ...
    if (await backToEditor.isVisible().catch(() => false)) {
      ...
      await page.getByText('角色概念分解圖').first().click();
  ...
  test('banks search filters options', async ({ page }) => {
    ...
    await bankSearch.fill('role');
    await expect(bankSearch).toHaveValue('role');
  });
